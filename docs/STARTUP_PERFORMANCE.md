...
1. **Lazy-load progress dialogs and Settings.** [App.tsx](../src/App.tsx) imports `DescribeProgressDialog`, `GeocodeProgressDialog`, `NormaliseProgressDialog`, `SettingsDialog`, `ColumnSelectionDialog`, `ApplyProgressDialog`, and `TargetVerifyOutcomeDialog` at the top level. All are modal / non-critical for first paint. `React.lazy()` + `Suspense` would shrink the critical module graph. List search no longer adds a frontend worker module because matching is owned by the Rust session service.
