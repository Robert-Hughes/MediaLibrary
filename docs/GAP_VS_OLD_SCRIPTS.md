...
Legacy: **single Gemini call per batch, image+text together** (`Update Metadata (One Batch).txt` lines 195-245). Required visual inspection of full-size originals (not thumbs). Geocode runs *first* as separate Python call so location strings already in agent context when it views images. Multi-batch chaining used cumulative "summary for next run" appended to next prompt for theme continuity.
...
|---|---|---|
| **GPS clustering detection** | Manually spotted ("all London photos at one coord" — TODO.md known issue, partly attempted via per-image visual rewrite) | **MISSING**. Haversine util exists in `src-tauri/src/geocode_cache.rs:70-84` but only for cache match. No outlier/cluster scan. |
...
| **Geocode cache** | per-batch + global cache, 50m haversine match, Nominatim 1s ratelimit | **PRESENT**. `geocode_cache.rs` better than legacy (per-host rate buckets, retry middleware, Overpass enrichment). |
| **Geocoding city/state bug** | TODO.md: some 2010 photos got "England" stored as City instead of State | **UNKNOWN**. Worth checking new mapping in `geocode.rs` against legacy bug. |
...
| **Verification HTML** | `generate_verification_report.py` — re-reads metadata after apply, compares to proposal, PASS/FAIL per file | **MISSING**. App has per-tag `TagOutcome` (Match/Coerced/Mismatch) in `apply_edits.rs:58-76` but no aggregated verification view. JSONL log forensic-only. |
| **Correction batches** | `batch-CORRECTION-Hawnby`, `-FINAL`, `-ADDITIONAL` — re-process flagged subsets | **MISSING**. Failed files logged (`apply_edits.rs:38` `FailedFile`) but no "retry failed subset" UI. |
...
| **Audit trail** | Per-batch logs `_review_HHMMSS.log`, `_apply_HHMMSS.log` | **PRESENT** (JSONL `MediaLibraryApplyLog.jsonl` per file, schema v3, before/after/outcome) — actually richer than legacy. |
...
1. **GPS cluster scan**: scan library, find lat/lon points with N>threshold photos within R meters, flag as "suspected manual bulk tag", offer per-image visual re-geocode.
