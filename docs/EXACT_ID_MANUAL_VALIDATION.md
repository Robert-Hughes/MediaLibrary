...
10. **Inspect a collision.** In Add New Property, search for a known repeated
    friendly name such as `File:BMPVersion`. Expected: distinct table contexts
    are visible and selecting one never substitutes its sibling.
11. **Try unknown/read-only fields.** Expected: editor and apply actions are
    unavailable; missing schema is never treated as writable.
12. **Inspect apply/readback outcomes.** Apply a mixed set of edits. Expected:
    every success, mismatch or retained draft names the intended exact table,
    tag ID and optional index.
