...
function groupBehaviourSummary(g: NormaliseGroup): string {
  switch (g) {
    ...
    case "headline":
      return "Writes target fields: XMP-fileshop:Headline and IPTC:Headline. Behaviour: Picks fileshop headline if present, otherwise IPTC headline. If both are empty, no drafts. No AI.";
    case "title":
    ...
    case "location":
      return "Writes target fields: XMP-iptcCore:Location, IPTC:Sub-location, XMP-fileshop:City, IPTC:City, XMP-fileshop:State, IPTC:Province-State, XMP-fileshop:Country, IPTC:Country-PrimaryLocationName, XMP-iptcCore:CountryCode, and IPTC:Country-PrimaryLocationCode. Behaviour: Synchronises XMP↔IPTC mirror pairs; XMP side wins on disagreement. No reverse-geocoding. If both sides of a pair are empty, that pair emits no drafts. No AI.";
    case "dates":
      return "Writes target fields: ExifIFD:DateTimeOriginal, ExifIFD:CreateDate, XMP-fileshop:DateCreated, IPTC:DateCreated, IPTC:TimeCreated, XMP-xmp:CreateDate, IPTC:DigitalCreationDate, and IPTC:DigitalCreationTime. Reads offset/subsecond fields (ExifIFD:OffsetTimeOriginal, ExifIFD:SubSecTimeOriginal, ExifIFD:OffsetTimeDigitized, ExifIFD:OffsetTime, ExifIFD:SubSecTimeDigitized) and filename fallback input. Behaviour: Normalises dates to ISO 8601; EXIF primary wins on conflict. Falls back to filename matching for shutter time if all shutter-time fields are empty. No AI.";
    case "description":
