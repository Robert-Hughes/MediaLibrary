...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div style={{ padding: 24, fontFamily: "sans-serif", color: "#eee", background: "#222", height: "100vh", overflow: "auto" }}>
        <h2 style={{ marginTop: 0 }}>Something went wrong in {this.props.name}</h2>
        <pre style={{ whiteSpace: "pre-wrap", color: "#f88" }}>
          {error.name}: {error.message}
        </pre>
        {error.stack && (
          <pre style={{ whiteSpace: "pre-wrap", fontSize: 12, opacity: 0.75 }}>{error.stack}</pre>
        )}
