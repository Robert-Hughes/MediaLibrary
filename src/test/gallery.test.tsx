...
describe("PhotoList interaction", () => {
  function renderList(photos: PhotoInfo[], onPhotoOpen: (i: number) => void, onSelect: (i: number | null) => void) {
    ...
    render(<PhotoList photos={photos} thumbnails={thumbs} imageMetadata={imageMetadata} visibleColumns={[{ key: "date_modified", kind: "os" }, { key: "date_created", kind: "os" }]} sortConfig={{ primary: null, secondary: null }} onSortChange={() => {}} selectedIndex={null} onSelect={onSelect} onShowInExplorer={() => {}} onVisibilityChange={() => {}} onPhotoOpen={onPhotoOpen} onSelectColumns={() => {}} />);
  }
