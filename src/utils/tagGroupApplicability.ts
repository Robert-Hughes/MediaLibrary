...
const IMAGE_GROUPS = new Set<string>([
  "IFD0", "IFD1", "ExifIFD", "GPS", "InteropIFD", "SubIFD", "SubIFD1", "SubIFD2",
  "MakerNotes", "IPTC", "Photoshop", "JFIF", "Adobe", "AdobeCM", "Ducky",
  "PNG", "PNG-pHYs", "PNG-tEXt", "PNG-iTXt", "PNG-zTXt",
  "GIF", "BMP", "WebP", "HEIC", "HEIF", "AVIF", "JPEG", "JPEG-2000", "JP2",
  "JUMBF", "MPF0", "MPImage1", "MPImage2",
  "CanonRaw", "CanonVRD", "CanonCustom", "Canon",
  "Nikon", "NikonCapture", "NikonScan", "NikonSettings",
  "Sony", "Olympus", "Pentax", "Panasonic", "Fujifilm", "FujiFilm",
  "Minolta", "Sigma", "Samsung", "Sanyo", "Casio", "Ricoh", "Apple", "GE", "DJI",
  "DNG", "AdobeDNG", "CameraIFD",
  "GeoTIFF", "EXIF",
  "MPF", "Stim", "FLIR",
]);

const AUDIO_GROUPS = new Set<string>([
  "ID3", "ID3v1", "ID3v1_Enh", "ID3v2_2", "ID3v2_3", "ID3v2_4",
  "Vorbis", "FLAC", "Theora", "Opus", "Ogg",
  "MPEG", "AAC", "AC3", "APE", "Audible", "AIFF",
  "RIFF", "RIFF-info", "RIFF-exif", "WAV",
  "MPC", "Lyrics3", "ASF",
  "iTunes",
]);

const VIDEO_GROUPS = new Set<string>([
  "QuickTime", "Keys", "ItemList", "UserData", "Track1", "Track2", "Track3",
  "Track4", "Track5", "Track6", "AudioKeys",
  "Matroska", "MPEG", "M2TS", "ASF", "WMV", "MXF", "RIFF",
  "Theora", "AVI1", "DV", "Flash", "FlashPix",
]);
...
const IMAGE_EXTS = new Set<string>([
  ...
  "heic", "heif", "avif", "webp",
  "cr2", "cr3", "nef", "nrw", "arw", "rw2", "raf", "orf", "pef", "dng", "raw",
  "psd", "psb",
]);

const AUDIO_EXTS = new Set<string>([
  "mp3", "flac", "ogg", "oga", "opus", "m4a", "aac", "ape", "aiff", "aif",
  "wav", "wma", "mpc", "ac3",
]);

const VIDEO_EXTS = new Set<string>([
  "mp4", "m4v", "mov", "qt", "3gp", "3g2",
  "mkv", "webm", "avi", "wmv", "asf", "mts", "m2ts", "ts",
  "mxf", "flv", "f4v",
]);
 ...
 */
export function filterTagsByFilename(tags: readonly string[], filename: string | undefined): string[] {
  if (!filename) return [...tags];
  const cat = categoryOf(extOf(filename));
  if (cat === null) return [...tags];
  const allowed = cat === "image" ? IMAGE_GROUPS : cat === "audio" ? AUDIO_GROUPS : VIDEO_GROUPS;
